...
pub fn hash(path: &Path, config: &HashActionOptions) -> Option<HashEntry> {
    ...
    let mut f = std::fs::File::open(&path).unwrap().take(config.max_size);
    let mut hasher = Hasher::new();
    match std::io::copy(&mut f, &mut hasher){
        Ok(read) => { if read != metadata.len(){
            warn!("failed to read all data from: {}, {} bytes were read, but {} was expected", path.display(), &read, metadata.len());
            return None;
        }}
        Err(err) => {
            warn!("failed to copy data from: {}. Error: {}", path.display(), &err);
        }
    }
...
mod tests {
    ...
    fn test_basic_scenario() {
        ...
        let result = hash(&file, &HashActionOptions{
            max_size: 14,
            oversized_file_policy: OversizedFilePolicy::Skip,
            collect_ext_attrs: false
        }).unwrap();

        assert_eq!(result.sha1.unwrap(), "a62a6d5991238ae72d81fe6e4769b3043d9fe670".as_bytes().to_vec());
        assert_eq!(result.sha256.unwrap(), "d76d85adca8afad205edebc11f9b5086bca75acb512a748bc79660e1346af546".as_bytes().to_vec());
        assert_eq!(result.md5.unwrap(), "e091b6f1a233049d22d2807fa8086f3f".as_bytes().to_vec());
        assert_eq!(result.num_bytes.unwrap(), 14);
    }

    // TODO: add tests for all the cases
}
